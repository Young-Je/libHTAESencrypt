...
{
  register uint8_t i = 16;
    while (i--) static_cast<void>(buf[i] ^= (cpk[i] = key[i])), cpk[16+i] = key[16 + i];
}
...
void aes_expandEncKey(uint8_t *k, uint8_t *rc) 
{
  ...
  k[3] ^= rj_sbox(k[28]);
  *rc = F( *rc);
    for(i = 4; i < 16; i += 4) static_cast<void>(k[i] ^= k[i-4]),  static_cast<void>(k[i+1] ^= k[i-3]),
        static_cast<void>(k[i+2] ^= k[i-2]), k[i+3] ^= k[i-1];
  k[16] ^= rj_sbox(k[12]);
  k[17] ^= rj_sbox(k[13]);
  k[18] ^= rj_sbox(k[14]);
  k[19] ^= rj_sbox(k[15]);
    for(i = 20; i < 32; i += 4) static_cast<void>(k[i] ^= k[i-4]),  static_cast<void>(k[i+1] ^= k[i-3]),
        static_cast<void>(k[i+2] ^= k[i-2]), k[i+3] ^= k[i-1];
}
void aes_expandDecKey(uint8_t *k, uint8_t *rc) 
{
  uint8_t i;
    for(i = 28; i > 16; i -= 4) static_cast<void>(k[i+0] ^= k[i-4]), static_cast<void>(k[i+1] ^= k[i-3]),
        static_cast<void>(k[i+2] ^= k[i-2]), k[i+3] ^= k[i-1];
  k[16] ^= rj_sbox(k[12]);
  k[17] ^= rj_sbox(k[13]);
  k[18] ^= rj_sbox(k[14]);
  k[19] ^= rj_sbox(k[15]);
    for(i = 12; i > 0; i -= 4) static_cast<void>(k[i+0] ^= k[i-4]), static_cast<void>(k[i+1] ^= k[i-3]),
        static_cast<void>(k[i+2] ^= k[i-2]), k[i+3] ^= k[i-1];
  *rc = FD(*rc);
  k[0] ^= rj_sbox(k[29]) ^ (*rc);
  k[1] ^= rj_sbox(k[30]);
...
void aes256_encrypt_ecb(aes256_context *ctx, uint8_t *buf)
{
  ...
  for(i = 1, rcon = 1; i < 14; ++i)
  {
    ...
    else static_cast<void>(aes_expandEncKey(ctx->key, &rcon)), aes_addRoundKey(buf, ctx->key);
  }
  aes_subBytes(buf);
  aes_shiftRows(buf);
...
int Do_Aes256(unsigned char* Src, unsigned char* Key, unsigned char* Dest, unsigned char flag)
{
	...
	aes256_init(&ctx, Tempkey);
	if(flag == 'e' || flag == 'E')
	{
		aes256_encrypt_ecb(&ctx, Tempbuf);
	}
	else
	{
		aes256_decrypt_ecb(&ctx, Tempbuf);
	}
